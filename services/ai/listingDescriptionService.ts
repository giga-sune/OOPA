...
function readErrorProperty(
  error: unknown,
  property: FirebaseAIErrorProperty,
): unknown {
  if (typeof error !== "object" || error === null || !(property in error)) {
    return undefined;
  }

  return (error as Record<FirebaseAIErrorProperty, unknown>)[property];
}

function logFirebaseAIError(error: unknown): void {
  if (!__DEV__) return;

  const cause = readErrorProperty(error, "cause");

  console.error("FIREBASE AI ERROR DETAILED:", {
    rawError: error,
    ...
    stack: readErrorProperty(error, "stack"),
    cause:
      cause === undefined
        ? undefined
        : {
            rawError: cause,
            name: readErrorProperty(cause, "name"),
            message: readErrorProperty(cause, "message"),
            code: readErrorProperty(cause, "code"),
            details: readErrorProperty(cause, "details"),
            stack: readErrorProperty(cause, "stack"),
          },
  });
}
